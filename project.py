...
def tarikh_():
    ...
    def search():
        ...
        if len(search_entry.get()) == 11:
            ...
            ent_sys_serial.insert(END,"سریال دستگاه")
            ent_sys_serial.grid(row=0,column=6,sticky=E,)
            ent_sys_war = CTkEntry(s,corner_radius=0,font=font_enry,justify=CENTER)
            ent_sys_war.insert(END,"گارانتی")
            ent_sys_war.grid(row=0,column=5,sticky=E,)
            ent_name = CTkEntry(s,corner_radius=0,font=font_enry,justify=CENTER)
            ent_name.insert(END,"صاحب دستگاه")
            ent_name.grid(row=0,column=4,sticky=E,)
            ent_name_code = CTkEntry(s,corner_radius=0,font=font_enry,justify=CENTER)
            ent_name_code.insert(END,"کدملی")
            ent_name_code.grid(row=0,column=3,sticky=E,)
            ent_name_tel = CTkEntry(s,corner_radius=0,font=font_enry,justify=CENTER)
            ent_name_tel.insert(END,"شماره موبایل")
            ent_name_tel.grid(row=0,column=2,sticky=E,)
            ent_name_tel_sabet = CTkEntry(s,corner_radius=0,font=font_enry,justify=CENTER)
            ent_name_tel_sabet.insert(END,"تلفن ثابت")
            ent_name_tel_sabet.grid(row=0,column=1,sticky=E)
            ent_sys_arrvial = CTkEntry(s,corner_radius=0,font=font_enry,justify=CENTER)
            ent_sys_arrvial.insert(END,"تاریخ ورود")
    ...
    def search_3():
        ...
        if len(search_entry_3.get()) == 11:
            ...
            ent_name_tel_sabet.grid(row=0,column=1,sticky=E)
            ent_sys_arrvial = CTkEntry(s,corner_radius=0,font=font_enry,justify=CENTER)
            ent_sys_arrvial.insert(END,"تاریخ خروج")
            ent_sys_arrvial.grid(row=0,column=0,sticky=E,)
            list_sel = [ent_id,ent_sys,ent_sys_type,ent_sys_model,ent_sys_serial,ent_sys_war,ent_name,ent_name_code,ent_name_tel,ent_name_tel_sabet,ent_sys_arrvial]
            for sell in list_sel:
                sell.configure(state=DISABLED)
            for row in c.execute(f'SELECT * FROM paziresh WHERE tel={search_entry_3.get()} AND cost!=0'):
                a=row[0]+1590
                ...
                ent_name_tel_sabet.grid(row=0,column=1,sticky=E,)
                ent_sys_arrvial = CTkEntry(s,font=font_enry,justify=RIGHT)
                ent_sys_arrvial.insert(END,row[2])
                ent_sys_arrvial.grid(row=0,column=0,sticky=E,)
                list_sel = [ent_id,ent_sys,ent_sys_type,ent_sys_model,ent_sys_serial,ent_sys_war,ent_name,ent_name_code,ent_name_tel,ent_name_tel_sabet,ent_sys_arrvial]
                for sell in list_sel:
                    sell.configure(state=DISABLED)
